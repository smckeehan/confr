...
@section('content')
<h3 style="float:left"><a href="/">{!! Auth::user()->instance->name !!}</a> > {!! $community->name !!}</h3>
<div class="newsfeed-buttons">
  <a class="btn btn-primary" href="/community/{!! $community->id !!}/new" role="button"><span class="glyphicon glyphicon-plus"></span> New Post</a>
</div>
...
@if($posts->count() == 10)
  <div class="text-center">
    @if(Input::get('p') > 0)
      <a href="/community/{!! $community->id !!}?p={!! Input::get('p') - 1 !!}" role="button" class="btn btn-default"><span class="glyphicon glyphicon-chevron-left"></span> Previous</a>
    @endif
    <a href="/community/{!! $community->id !!}?p={!! Input::get('p') + 1 !!}" role="button" class="btn btn-primary">Next <span class="glyphicon glyphicon-chevron-right"></span></a>
  </div>
@endif
